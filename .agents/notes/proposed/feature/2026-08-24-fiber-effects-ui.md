...
Fiber Inspector 会从“生命周期身份/依赖摘要”扩展到“Cordis 已标注的 live lifecycle effects”。展示仍是 observer-only；没有 listener wrapping、额外 RPC、poller、Host history 或原始 disposer/config/args 暴露。深层树需要用户逐层展开，但能避免默认界面过密。
...
- `pnpm verify:policy`、typecheck、test、build、client-bundle 全绿；
- real DSH Web smoke 继续通过；
- TS/TSX 显式 `any` 扫描为 0。
