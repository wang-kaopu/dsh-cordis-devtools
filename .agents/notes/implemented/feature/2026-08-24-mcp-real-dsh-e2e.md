...
- `cordis_inspect_event` reports exactly two current listeners with two distinct live owner uids;
- `cordis_inspect_fiber` by name returns both live Fibers and by uid resolves each authoritative instance with ownership of the duplicate event;
- `cordis_search_dispatches` returns bounded recent evidence involving one of those Fiber uids.
...
The E2E now proves the exact external-agent boundary and the duplicate-Fiber debugging evidence chain without requiring Codex/Claude credentials. It adds one temporary loopback listener, one E2E-only runtime fixture, and one temporary patch file inside the disposable test environment. Current listener/Fiber facts remain authoritative while dispatch observations remain explicitly bounded recent evidence.
