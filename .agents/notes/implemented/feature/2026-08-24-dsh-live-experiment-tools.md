...
A DSH Host with ToolRuntime now discovers the mutation tools automatically, but actual start authority is still per-call and one-shot. A pure Cordis runtime sees no hard failure when ToolRuntime is absent. Tool disposal follows the injected DSH lifecycle, while any active instrumentation lease remains owned and cleaned by `DevtoolsService` rather than by the tool registrations themselves.
