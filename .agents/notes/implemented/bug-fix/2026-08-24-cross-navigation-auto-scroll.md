...
The panel header also had an asymmetric action treatment: Close was icon-only while Refresh carried visible text, making the two adjacent actions visually inconsistent.
...
Refresh is reduced to the same icon-only ghost-button treatment as Close. It keeps the existing refresh icon, tooltip, disabled/loading behavior, and an explicit accessible label; only the visible `Refresh` text is removed.
...
- Keep Refresh as a labeled toolbar button. Rejected because Refresh and Close form the same compact header action group and the tooltip/accessibility label already preserve meaning.

## Consequences

Cross-navigation now keeps the selected Event/Fiber visible in its left list without adding routing state, timers, or layout assumptions. Refresh and Close share a compact icon-only header treatment without losing accessibility. Both changes are browser-presentation-only and do not affect Host/runtime semantics.
